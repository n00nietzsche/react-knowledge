...
이제 `jwt`라는 password-jwt strategy로 이동하게 됩니다. 이것은 어플리케이션 내부 보호된 routes위에서 불려지는 인증입니다: `findUsers`, `updateUser`, `deleteUser`. jwt strategy를 위해, JWT는 클라이언트의 호출에 각각 넘겨집니다. (위의 코드에서는 `JWT`라는 인증 헤더로 넘겼습니다.), 후에 시크릿을 이용하여 JWT가 추출되고 디코딩 됩니다.(우리가 주로 .env파일에 저장해뒀던 그 secret을 말합니다. 이 포스트에서는 jwtConfig.js에 저장해뒀습니다.)
